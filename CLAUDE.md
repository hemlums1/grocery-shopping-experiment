...
**Verified behaviorally** (jsdom, real `input` events dispatched against the actual rendered page and the actual `search.js` — not just "the markup looks right"): exact-category matches (`"apples"` → 4 items, only Produce visible), cross-category matches (`"organic"` → 34 items across all 6 categories), matches on fields other than name/brand (`"ecuador"` correctly matched not just the 4 Ecuadorian banana variants but also a dark chocolate bar whose cocoa is sourced from Ecuador — a real cross-check of the data, not just the matching logic), the empty-state message, case-insensitivity, and whitespace-only input correctly treated as "no filter." This is the one client-side feature this session that *didn't* turn up a bug during testing.
