...
A search box in the toolbar (`/` only — `/basket` and `/review` show too few items for search to matter) filters the catalog live as you type, via `public/js/search.js`. Purely client-side: all 104 items are already rendered in the DOM (no pagination), so filtering is just toggling `hidden` on `.item` elements — no server round-trip, no debounce needed.

Each item gets a precomputed `data-search` attribute (name + brand + product type + category + sourcing practice + country, lowercased) at render time, rather than reading `.textContent` live — that keeps the matched fields deliberate (e.g. excludes the price and the single-letter Nutri-Score badge, which would otherwise make searching "a" match nearly everything). A category section hides itself if none of its items match, and a "No items match your search" message appears if literally nothing does. Composes independently with sort (order) and the AJAX basket updates (quantity text) — neither touches `hidden` or `data-search`, so there's no interaction to worry about.

**A real bug Gavin caught that jsdom testing missed entirely:** searching correctly hid non-matching *categories* but left non-matching *items within a visible category* on screen (e.g. searching "potatoes" correctly hid Dairy/Meat/etc., but Apples and Spinach stayed visible alongside Potatoes within Produce). Root cause: `.item { display: flex; }` and the browser's default `[hidden] { display: none; }` are the *same CSS specificity* (one class selector vs. one attribute selector) — author stylesheets load after the UA default, so `display: flex` won the cascade and silently neutralized the `hidden` attribute for every `.item`, even though `search.js` was setting it correctly. `.category` had no competing `display` rule, which is exactly why category-level hiding worked while item-level hiding didn't — a very specific, easy-to-miss split. Fixed the same way the lightbox already (correctly, if accidentally) handled this: an explicit `.item[hidden] { display: none; }`, which has strictly higher specificity and wins regardless of declaration order.

**Why the original "behaviorally verified" claim for this feature was wrong:** the jsdom tests checked `item.hidden` (the DOM attribute/property) and never `getComputedStyle(item).display` (the actual visual result) — so they could only ever catch JS logic bugs, not CSS cascade bugs. Worse, when this was investigated, jsdom's computed-style output was *identical* whether tested against the buggy CSS or the fixed CSS — its CSS engine doesn't reliably model this kind of same-specificity cascade conflict at all, so it's not just that the original tests missed this, it's that this whole category of bug isn't really testable via jsdom. Confirming the bug and the fix both required Claude in Chrome, which finally connected after roughly eight failed attempts across this session — real visual confirmation, not an attribute check standing in for one. Lesson for next time: for anything where `hidden`/`display` toggling is involved, either get real browser confirmation or proactively add the `[hidden]` override the moment a `display` property is set on the same selector, rather than waiting to be proven wrong.

## AJAX Basket Updates (No Page Reload)

Quantity +/- on `/` and `/basket` no longer does a full page navigation. `public/js/basket.js` intercepts the `.qty-form` submit, sends the same `POST /basket/update` via `fetch` instead, and patches the DOM from the JSON response (`{ itemId, quantity, total, budgetCents, basketCount }`) — updating that item's qty/buttons, *every* item's increment-disabled state (another item's afford­ability can change after any add), the header budget line, the "Basket (N)" link, and (on `/basket`) the line-total text and the footer total. If an item hits quantity 0 on `/basket`, its row is removed from the DOM entirely (and the empty-basket message is inserted if that was the last one) rather than left showing "0".

**Progressive enhancement, not a replacement:** `POST /basket/update` branches on the `Accept` header (`req.accepts(['html','json'])`) — a plain form submission (no JS, or fetch failure) still gets the original redirect behavior. If `fetch` ever rejects, `basket.js` falls back to `form.submit()` (a real navigation) rather than leaving the UI stuck. The hard budget cap is still enforced server-side as the only source of truth; a rejected increment just comes back with the *unchanged* quantity, which naturally keeps the UI in sync without the client needing to know "rejected" vs. "succeeded" as a separate signal.

**A real bug found during testing, not just a test artifact:** the initial version sent the fetch body as a raw `FormData`, which makes the browser encode it as `multipart/form-data` — but `express.urlencoded()` only parses `application/x-www-form-urlencoded`, so `req.body` came back empty and every request 400'd. Fixed by converting to `URLSearchParams` before sending (`new URLSearchParams(formData)`), which fetch correctly encodes as the form-urlencoded type the server expects.

**A second real bug, reported by Gavin after the above shipped:** "nothing adds to the basket, and the screen still refreshes every time" — the fallback path itself was broken. `form.submit()` doesn't simulate a button click, so when fetch failed for *any* reason, the fallback submission silently dropped the increment/decrement action field; the server received no action, made no change, and the page navigated anyway — exactly the reported symptom. Fixed by pinning the clicked action into a real hidden input before calling the fallback submit, and by setting it unconditionally on `formData` up front (not only when `e.submitter` happens to be present). Also added explicit `credentials: 'same-origin'` on the fetch call to close off one plausible reason fetch could have been failing in the first place, and `console.error` logging so a future failure is visible rather than silent. **Caveat: the exact original trigger for fetch failing was never directly observed** (no browser console access this session either) — the fallback-losing-the-action bug fully explains the reported symptom regardless of what triggered it, but if this recurs, check the browser console for the logged error first.

**Verified behaviorally against the real local server**, not just structurally: loaded the actual rendered page into a jsdom sandbox (temporary `--no-save` dependency, removed after, never touched `package.json`), bridged jsdom's `fetch`/cookies to genuinely hit `http://localhost:3000`, and simulated real clicks via `form.requestSubmit(button)` (the standards-compliant way to get a correctly-populated `submitter`). Confirmed: quantities update, cross-item button states recompute correctly, the hard cap is enforced exactly at the boundary (verified the *exact* dollar amount where a third $14.99 item gets rejected), basket-page row removal and the empty-state transition both work, and the page genuinely never navigates. Two false leads worth knowing about if this needs re-testing later: (1) jsdom's `URLSearchParams` fails Node's `fetch`'s `instanceof` check across the jsdom/Node realm boundary, which looked like a body-encoding bug but was purely a test-bridge artifact; (2) loading `basket.js` both via the page's own `<script src>` tag (jsdom does execute these when `resources:'usable'`) *and* via an extra manual `eval` double-registered the submit listener, which looked like a quantity-doubling bug but was also purely a test-setup mistake, not a problem in the shipped code. Still outstanding, same as the rest of this UI: a human actually looking at it in a real browser.
...
| AJAX vs. full SPA rewrite for basket updates | Progressive enhancement on the existing server-rendered forms (intercept + `fetch`, JSON response, redirect fallback) | Smallest change that removes the reload; a client-side framework or full SPA rewrite would contradict the project's standing "no framework" decision for a problem this scoped |
| Basket update response shape | Always return current ground truth (`quantity`, `total`, etc.), even when an action was rejected | Client doesn't need a separate success/failure signal — the UI just renders whatever the server says is true, which is simpler and can't drift out of sync |
| Search implementation | Client-side filtering over the already-rendered DOM, not a server query | All 104 items are already on the page with no pagination, so there's nothing a server round-trip would add except latency |
| Searchable fields | Precomputed `data-search` attribute (name, brand, product type, category, sourcing, country) rather than matching live `.textContent` | Deliberate allowlist — keeps price text and the single-letter Nutri-Score badge out of the match, which would otherwise make short/numeric queries match almost everything |
| Item-level search hiding | `.item[hidden] { display: none; }` (higher specificity than `.item { display: flex; }`), not `!important` | Same fix pattern already in place for `.lightbox[hidden]`; specificity is the more maintainable tool than `!important` since it doesn't block future overrides if `.item`'s display ever needs to change |
