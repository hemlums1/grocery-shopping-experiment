...
- Real product photos live in `public/images/items/` (one image per **product type**, shared across all of that type's brand/organic/ethics variants — e.g. every orange juice variant uses the same photo), copied from the source files Gavin provided in `Item Pictures/`.
